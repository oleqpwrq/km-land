...
function CallButton({ className = "" }: { className?: string }) {
  return (
    <a
      ...
      className={cx(
        "rounded-xl px-4 py-2 text-sm font-semibold shadow border border-white/20 hover:bg-white/10 shrink-0 whitespace-nowrap",
        className
      )}
    >
      Позвонить
...
function validateEmailOptional(v: string) {
  return !v || /^[^\s@]+@[^\s@]+\.(ru|com)$/i.test(v);
}
...
export default function Page() {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    setMessage(null);
    if (!nameOk || !phoneOk || !emailOk || !agree) {
  ...
  return (
    <div className="min-h-screen">
      {/* HEADER: на мобиле всё в одну линию с логотипом; кнопки не наезжают (truncate у названия, кнопки shrink-0) */}
      <header className="sticky top-0 z-40 w-full border-b border-white/10 bg-[#0e1114]/70 backdrop-blur">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 md:px-8 h-16">
          <div className="flex h-full items-center gap-2">
            {/* Лого + имя */}
            <div className="flex items-center gap-2 min-w-0">
              <Image src="/logo.png" alt="KMLeads" width={32} height={32} className="rounded shrink-0" />
              <span className="font-semibold truncate text-sm sm:text-base">KMLeads</span>
            </div>

            {/* Кнопки справа: одна линия, без схлопывания */}
            <div className="ml-auto flex items-center gap-2">
              <a
                href="#form"
                className="rounded-xl px-3 py-2 text-sm font-semibold shadow shrink-0 whitespace-nowrap"
                style={{ background: accent }}
              >
                Оставить заявку
              </a>
              <CallButton className="px-3 py-2 text-sm" />
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="grid items-center gap-10 md:grid-cols-2">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <a
                  href="#form"
                  className="rounded-2xl px-6 py-3 text-base font-semibold shadow-lg"
                  style={{ background: accent }}
                >
                ...
                <a
                  href="#how"
                  className="rounded-2xl px-6 py-3 text-base font-semibold border border-white/20"
                >
                  Как это работает
                </a>
            ...
            >
              <div className="aspect-[4/3] w-full overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-1">
                <div className="relative h-full w-full overflow-hidden rounded-2xl bg-[#0b0e11] grid place-items-center">
                  <div className="text-center">
                    <div className="text-sm text-white/50">Realtime feed</div>
                    <div className="mt-2 text-2xl md:text-3xl font-semibold">5+ млн публикаций ежедневно</div>
                    <div className="mt-2 text-white/70 text-sm">Отслеживаем более 200 000 Telegram-чатов</div>
                  </div>
                </div>
              </div>
              <div
                className="pointer-events-none absolute -bottom-8 right-0 h-40 w-40 rounded-full blur-2xl opacity-60"
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-6 md:px-8 py-16 md:py-24">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-8">
              ...
              <div className="mt-3 text-4xl font-extrabold" style={{ color: accent }}>
                20 000 ₽ / мес
              </div>
          ...
          <p className="mt-6 text-sm text-white/50">
            *Не является публичной офертой. Цены действительны на день отправки КП.
          </p>
        </div>
      </section>
          ...
          <div className="grid gap-8 md:grid-cols-2">
            <div>
              ...
              <p className="mt-5 text-white/80 max-w-xl">
                Оставьте заявку — подключим KMLeads, настроим ключевые запросы и запустим поток лидов уже сегодня.
              </p>
                ...
                <div className="grid gap-5">
                  <div>
                    ...
                    <input
                      ...
                      className={cx(
                        "mt-2 w-full rounded-xl bg-[#0b0e11] px-4 py-3 outline-none ring-1 ring-inset",
                        nameOk ? "ring-white/10" : "ring-red-500/60"
                      )}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className={cx(
                        "mt-2 w-full rounded-xl bg-[#0b0e11] px-4 py-3 outline-none ring-1 ring-inset",
                        phoneOk ? "ring-white/10" : "ring-red-500/60"
                      )}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="name@company.ru | name@company.com"
                      className={cx(
                        "mt-2 w-full rounded-xl bg-[#0b0e11] px-4 py-3 outline-none ring-1 ring-inset",
                        emailOk ? "ring-white/10" : "ring-red-500/60"
                      )}
                    />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Кратко опишите задачу, нишу и KPI"
                  ...
                  <button
                    type="submit"
                    disabled={!nameOk || !phoneOk || !emailOk || !agree || loading}
                    className="rounded-2xl px-6 py-3 font-semibold shadow-lg disabled:opacity-50"
                    style={{ background: accent }}
                  >
      ...
      {/* FOOTER: одна линия даже на мобиле; если ширины не хватает — адрес обрезается (truncate) */}
      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 md:px-8 py-6">
          <div className="flex items-center justify-between gap-3 flex-nowrap">
            {/* Лого/название */}
            <div className="flex items-center gap-2 min-w-0">
              <Image src="/logo.png" alt="KMLeads" width={20} height={20} className="rounded shrink-0" />
              <div className="min-w-0">
                <div className="font-semibold text-white text-sm leading-5 truncate">KMLeads</div>
                <div className="text-[11px] text-white/70 leading-4 truncate">Корпоративный Маркетинг</div>
              </div>
            </div>

            {/* Адрес + звонок в одну линию; адрес может обрезаться */}
            <div className="flex items-center gap-2 min-w-0 flex-1 justify-center">
              <div className="truncate text-xs text-white/70 max-w-[46vw]">
                Москва, Проспект Мира 102к1, БЦ «Парк Мира»
              </div>
              <CallButton className="px-3 py-2 text-xs" />
            </div>

            {/* Telegram справа */}
            <div className="flex items-center gap-2 shrink-0">
              <a
                href="https://t.me/kmleads"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2 hover:bg-white/10 text-xs whitespace-nowrap"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path d="M22 4L2 11l6 2 2 6 12-15Z" stroke="white" strokeWidth="1.5" />
                </svg>
          ...
          <div className="pt-3 text-center text-[11px] text-white/50">
